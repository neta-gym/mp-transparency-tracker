...
class DebuggerAgent:
    ...
    async def check_model_roundtrip(self) -> SuiteResult:
        ...
        rf = ResearchFindings(
            ...
            assets=AssetDeclaration(
                ...
                previous_total_assets=500_000,
                asset_year=2024,
                previous_asset_year=2019,
            ),
    ...
    async def check_scoring_invariants(self) -> SuiteResult:
        ...
        suite.checks.append(
            _check("Attendance: minister with data → actual", calc_attendance_score(80, is_minister=True) == 80.0)
        )
        suite.checks.append(
            _check("Attendance: minister no data → 50.0", calc_attendance_score(None, is_minister=True) == 50.0)
        )
        suite.checks.append(
            _check(
        ...
        suite.checks.append(
            _check("Committee: capped at 100", calc_committee_score(5, 3, data_confidence=0.7) <= 100.0)
        )
        ...
        # --- Legislative ---
        suite.checks.append(
            _check("Legislative: (0,0,0) no data → 40.0 (neutral)", calc_legislative_score(0, 0, 0) == 40.0)
        )
        suite.checks.append(
            _check("Legislative: (0,0,0) with data → 0", calc_legislative_score(0, 0, 0, data_confidence=0.5) == 0.0)
        )
            ...
            for entry in lb.entries:
                ...
                if abs(expected - entry.composite_score) > 0.5:
                    sum_mismatches.append(
                        f"{entry.mp_name}: expected={expected:.2f}, actual={entry.composite_score:.2f}"
                    )
            suite.checks.append(
                _check(
    ...
    def _get_state_dirs(self) -> list[Path]:
        ...
        return [
            ...
            if d.is_dir() and d.name not in ("__pycache__", ".git") and (d / "raw").exists()
        ]
    ...
    def _render(self, results: list[SuiteResult]) -> None:
        ...
        failures = [
            (sr.suite_name, c) for sr in results for c in sr.checks if c.status in (CheckStatus.FAIL, CheckStatus.WARN)
        ]
        if failures:
